#!/usr/bin/env bash
# 03-01-mount-syscall チェッカー: パッチ適用後の6.6をビルドしQEMU起動、busybox initramfsで
...
rm -rf "$root" "$ir"
if grep -Fq "m3syscall:" "$log"; then echo PASS; rm -f "$log"; exit 0
else echo "FAIL: ブートログにマーカー(m3syscall:)が見つかりません"; rm -f "$log"; exit 1; fi
