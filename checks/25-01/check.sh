#!/usr/bin/env bash
# 25-01-rcu チェッカー: 期待マーカーが dmesg に出るか
set -u
...
[ -f rcudemo.ko ] || { echo "FAIL: rcudemo.ko が生成されていません"; exit 1; }
$SUDO rmmod rcudemo 2>/dev/null || true
$SUDO insmod rcudemo.ko 2>/dev/null || { echo "FAIL: insmod に失敗しました"; exit 1; }
hit=$($SUDO dmesg | tail -n 30 | grep -cE "m25rcu: callback freed val=42")
$SUDO rmmod rcudemo 2>/dev/null || true
